...
module.exports = class Component extends Module {
	constructor(moduleId, components) {
		super(moduleId, DEFAULT_CONFIG.FAMILY_KEYS.components, components);
	...
	_findName(moduleId) {
		...
		if (/template\.hbs$/.test(moduleId)) {
			return moduleId.replace(/^.+\/components\/(.+)\/template\.hbs$/, '$1');
		}
		// ember way
		return moduleId.replace(/^.+\/components\/(.+)\.hbs$/, '$1');
	}
	...
	_findContextuals(node, dictionary = {}) {
		if (node.name === 'yield' && node.contextuals) {
			...
			node.components.forEach((component, index) => {
				...
				if (component.name === 'yield' && (!component.contextuals || component.contextuals && component.contextuals.length)) {
