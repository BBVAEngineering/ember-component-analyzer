...
module.exports = class Analyzer {
	...
	process() {
		 ...
		 */
		const getComponentByPath = (cPath) => {
			const parts = cPath.split('.');

			return parts.reduce((parentName, next) => {
				const parent = this.modules[parentName];

				if (parent && parent.contextuals) {
					const dictionary = parent.contextuals;

					if (dictionary) {
						const name = dictionary[next];

						// TODO: loop name to retrieve all the valid contextuals
						return (name && name[0]);
					}
				}
			});
		...
		function replaceContextuals(node, dictionary = {}) {
			...
			if (parts.length > 1) {
				...
				const resolvedName = getComponentByPath(node.name);
